...
async function isBootstrapComplete(){ const m = await getJSON(BOOTSTRAP_K); return !!m?.bootstrapComplete; }
...
export function mountFirstRun(root){
  root.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="row">
        <div class="col-8"><input id="pwUnlock" type="password" placeholder="Master password"></div>
        <div class="col-4"><button id="btnUnlock" class="button">Unlock</button></div>
      </div>
    </div>
  `;
  ...
  async function showCorrectPane(){
    if (await isBootstrapComplete()) { $('#bootstrapCard').style.display='none'; $('#setupCard').style.display=''; }
    else                              { $('#bootstrapCard').style.display='';   $('#setupCard').style.display='none'; }
  }
  showCorrectPane();
  ...
  $('#bsBtn').addEventListener('click', async ()=>{
    ...
    try{
      ...
      showCorrectPane();
    } catch(e) { $('#bsMsg').textContent = e.message; }
  });
  ...
  $('#btnSave').addEventListener('click', async ()=>{
    ...
    try{
      await firstRunSetup(p1, window.__cp_handle);
      await saveNow();
      alert('Created. You can now use the Clinical tab.');
      location.hash = '#/clinical'; location.reload();
  ...
  $('#btnUnlock').addEventListener('click', async ()=>{
    ...
    try{
      ...
      alert('Unlocked.');
      location.hash = '#/clinical'; location.reload();
    }catch(e){ alert('Unlock failed. Check password or data file.'); }
  });
}
